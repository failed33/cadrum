...
fn test_fillet_cube_reduces_volume_and_area() {
	...
	assert!(rounded.volume().expect("volume integration") < original_volume, "fillet must reduce volume: {} vs {}", rounded.volume().expect("volume integration"), original_volume);
	assert!(rounded.area().expect("area integration") < original_area, "fillet must reduce area: {} vs {}", rounded.area().expect("area integration"), original_area);
}
...
fn test_fillet_cube_matches_analytical_volume() {
	...
	let rel_err = (rounded.volume().expect("volume integration") - expected).abs() / expected;
	assert!(rel_err < 1e-3, "rounded cube volume {} vs analytical {} (rel err {})", rounded.volume().expect("volume integration"), expected, rel_err);
}
...
fn test_fillet_empty_edges_is_noop() {
	...
	assert!((unchanged.volume().expect("volume integration") - original_volume).abs() < EPS, "no-op fillet should preserve volume exactly, got {}", unchanged.volume().expect("volume integration"));
}
