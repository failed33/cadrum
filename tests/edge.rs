...
fn loop_sizes(edges: &[Edge]) -> Result<Vec<usize>, cadrum::Error> {
	Edge::loops(edges).map(|groups| groups.iter().map(Vec::len).collect())
}

#[test]
fn loops_splits_outer_and_hole() {
	let outer = Edge::polygon(&[DVec3::ZERO, DVec3::X * 4.0, DVec3::new(4.0, 4.0, 0.0), DVec3::Y * 4.0]).unwrap();
	let hole = Edge::polygon(&[DVec3::new(1.0, 1.0, 0.0), DVec3::new(2.0, 1.0, 0.0), DVec3::new(2.0, 2.0, 0.0)]).unwrap();
	let profile = [outer, hole].concat();
	assert_eq!(loop_sizes(&profile).unwrap(), vec![4, 3]);
}

#[test]
fn loops_single_circle() {
	let circle = Edge::circle(2.0, DVec3::Z).unwrap();
	assert_eq!(loop_sizes(&[circle]).unwrap(), vec![1]);
}

#[test]
fn loops_empty() {
	assert_eq!(loop_sizes(&[]).unwrap(), Vec::<usize>::new());
}

#[test]
fn loops_rejects_trailing_open_group() {
	let square = Edge::polygon(&[DVec3::ZERO, DVec3::X, DVec3::X + DVec3::Y, DVec3::Y]).unwrap();
	let dangling = Edge::line(DVec3::Z * 5.0, DVec3::Z * 6.0).unwrap();
	let profile = [square, vec![dangling]].concat();
	assert!(loop_sizes(&profile).is_err(), "a trailing edge that never closes must not be silently dropped");
}

#[test]
fn loops_rejects_gap_inside_a_group() {
	let (a, b, c) = (DVec3::ZERO, DVec3::X, DVec3::Y);
	let broken = [Edge::line(a, b).unwrap(), Edge::line(b + DVec3::Z, c).unwrap(), Edge::line(c, a).unwrap()];
	assert!(loop_sizes(&broken).is_err());
}

#[test]
fn loops_agrees_with_is_loop_on_a_single_loop() {
	let square = Edge::polygon(&[DVec3::ZERO, DVec3::X, DVec3::X + DVec3::Y, DVec3::Y]).unwrap();
	assert!(Edge::is_loop(&square));
	assert_eq!(loop_sizes(&square).unwrap().len(), 1);
}
