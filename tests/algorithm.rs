...
fn a_radius_law_interpolates_the_fillet_between_its_stations() {
	...
	let rounded = |radius: f64, law: &[LawSample]| shape(apply(Algorithm::Fillet { shape: block.as_shape(), edges: &[edge], radius, law })).volume();
...
fn a_chamfer_by_two_distances_lays_the_first_on_its_reference_face() {
	...
	let kept: Vec<bool> = faces
		...
		.map(|reference| {
			...
			assert!((cut.volume() - expected).abs() < 1.0e-6, "chamfer volume {} vs {expected}", cut.volume());
			cut.contains(probe)
		})
		.collect();
...
fn a_chamfer_by_an_angle_rises_from_its_reference_face() {
	...
	assert!((bevelled.volume() - expected).abs() < 1.0e-6, "chamfer volume {} vs {expected}", bevelled.volume());
}
...
fn defeaturing_removes_a_blend_and_heals_the_edge_it_rounded() {
	...
	assert!((healed.volume() - block.volume()).abs() < 1.0e-6, "healed volume {} vs {}", healed.volume(), block.volume());
}
