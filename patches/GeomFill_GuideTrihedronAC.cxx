                                   ...
                                   gp_Vec&      D2BiNormal)
{
  ...
  d2tp_dt2 = (Orig2 - Orig1) * (Lguide / L) * DTo.Dot(To) / (NTo * NTG)
             - dtg_dt * dtg_dt * TG.Dot(DTG) / N2Tp;
...
// derivee seconde du triedre
  ...
  (void)DTDN;
#endif
  // The second derivative of a normalized vector uses the squared first
  // derivative, not the squared unit vector (which is always one).
  double TN2 = DTN.SquareMagnitude();
